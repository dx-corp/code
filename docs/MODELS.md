...
Nav: [Docs index](README.md) · [Quickstart](QUICKSTART.md) · [Safety](SAFETY.md) · [Model catalog](../packages/local-host-rs/src/model_catalog.rs)
...
catalog. The catalog snapshot (`packages/local-host-rs/src/model_catalog_data.json`)
is regenerated by the model-catalog pipeline from two sources:

- Native providers (Anthropic, OpenAI, Google, xAI) come from the
  [models.dev](https://models.dev) API (`https://models.dev/api.json`).
  Deprecated IDs such as `gpt-5.1-codex-max` are dropped on regeneration.
- OpenRouter rows come from OpenRouter's public
  `https://openrouter.ai/api/v1/models` catalog. Maestro keeps every current
  interactive route (`:batch` variants are omitted) and prefixes selections
  as `openrouter/<vendor>/<model>` so they do not collide with native
  provider ids. OpenRouter models stay on Chat Completions even when the
  nested vendor id would use Responses on the vendor's own API.

Runtime refresh pulls both sources independently at most once an hour and
never blocks startup. If one source is down, the other still updates. Opening
the TUI model selector reloads the on-disk cache so a completed refresh is
visible without restarting. The daily `model-catalog-freshness` workflow
regenerates the committed snapshot and opens `chore/model-catalog-refresh`
when the live catalogs have drifted. The runtime gateway hydrates
`GET /api/models` from the same public OpenRouter catalog when no LLM-gateway
catalog URL is configured.

Each entry reports protocol, tool use, vision, reasoning, streaming,
...
or `unknown` without rewriting capability metadata. An OpenRouter model id that
...
- **OpenRouter (Responses API):** reserved for the few OpenRouter families that
  still require `/responses`. The live OpenRouter catalog itself is Chat
  Completions; use `openrouter/<vendor>/<model>` for those routes. Historical
  overlays still include `openai/o4`, `openai/o4-mini`, and their `:online`
  variants on `https://openrouter.ai/api/v1/responses`.
