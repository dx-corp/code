(function (root, factory) {
  ...
  function routeString(scene, size, time, pinned, layout = "browse") {
    ...
    if (layout !== "browse") params.set("layout", layout);
    return `#${params.toString()}`;
  }
  function route(params, allowed) {
    const result = {};
    for (const key of ["scene", "size", "time", "mode", "layout"]) if (allowed[key]?.includes(params[key])) result[key] = params[key];
    return result;
  }
